...
```{r Using rsample::vfold_cv() to set up 10 fold Cross-Validation}
set.seed(234)
...
```{r [TRAINING] Updating the Map with Predictions}
...
The prediction diagram shows the model continues to accurately classify
the stations in the testing set (unseen stations), proving it can handle new data well.

```{r [TESTING] Updating the Map with Predictions}
# Function to determine the color based on the prediction being correct, incorrect, or not found
getPredictionColor <- function(station_name) {
  # Check if the station exists in the region_pred_summary
...
```{r Presenting the final working dataframe}
...
```{r Calculating a "Good Weather Score"}
...
```{r Normalising variables prior to running a GLM model}
...
```{=html}
<a href="https://prospectivepaycalc.s3.eu-west-2.amazonaws.com/Assignment_DSTI.zip" download="Assignment3_ProjectFiles.zip">
